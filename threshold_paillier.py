import random
from sympy import Symbol, expand, interpolate, solve
try:
...
args = args_parser()
DEFAULT_KEYSIZE = args.key_size # 给的参数默认是256
DEFAULT_KEYSIZE = 2048 # 这里试试调大
...
class PaillierPrivateKey(object):
    ...
    def split_key(self, n, k):
        assert k <= n, "Number of required shares should be less than total shares"

        lambda_key = self.public_key.n - (self.p + self.q - 1)  # λ = n - (p + q - 1)
        x = Symbol('x')
        random_coeffs = [random.SystemRandom().randrange(1, self.public_key.n) for _ in range(k - 1)]
        f = expand(lambda_key + sum([random_coeffs[i] * x ** (i + 1) for i in range(k - 1)]))

        shares = [(i, f.subs(x, i)) for i in range(1, n + 1)]
        return shares

    @staticmethod
    def reconstruct_key(shares, public_key):
        x = Symbol('x')
        f = interpolate(shares, x)
        lambda_key = int(f.subs(x, 0))

        p_plus_q = public_key.n - lambda_key + 1
        p_minus_q = isqrt(p_plus_q ** 2 - 4 * public_key.n)
        q = (p_plus_q - p_minus_q) // 2
        p = p_plus_q - q
